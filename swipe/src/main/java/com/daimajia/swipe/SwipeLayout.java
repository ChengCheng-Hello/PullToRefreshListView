        ...
        if (status == Status.Close) {
            for (View bottom : bottoms) {
                if (bottom != null && bottom.getVisibility() != INVISIBLE) {
                    // fix bug 个别机型适配问题，暂时解决办法
//                    bottom.setVisibility(INVISIBLE);
                }
            }
        } else {
            View currentBottomView = getCurrentBottomView();
            if (currentBottomView != null && !currentBottomView.isShown()) {
...
//        if(bottomViewRect == null)
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        if (mClickToClose && getOpenStatus() == Status.Open) {
            if (isTouchOnSurface(ev)) {
                return true;
            } /*else {
                close();// 点击滑出部分自动关闭
            }*/
        ...
        switch (ev.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                if (mClickToClose && getOpenStatus() == Status.Open && !isTouchOnSurface(ev) && !mHasMove) {
                    close();
                }
                mHasMove = false;
                mIsBeingDragged = false;
    ...
    private Rect computeBottomLayoutAreaViaSurface(ShowMode mode, Rect surfaceArea) {
        ...
        // fix bug 个别机型适配问题，暂时解决办法
        if (rect.left < 0 && rect.left > -mDragDistance) {
